...
- **Admin content persistence:** CMS pages read/write through `src/services/content.js` + `pageContent.js` + `ContentProvider` (`useContent('<pageKey>')` from `src/hooks/useContent.js` (homepage/about/services/gallery/seo/settings). Two exceptions: `FAQsCMS` is a dedicated module backed by the `faqs` service (see below) and `Enquiries` is backed by the `enquiries` service. Seeds come from the public `src/constants/*.js`; with Supabase configured, 7 pages (`homepage/about/services/contact/gallery/seo/settings`, see `SUPABASE_CONTENT_PAGES` in `pageContent.js`) persist to the `page_content` table with realtime sync, so CMS edits go live on the public site. Without `.env` they fall back to localStorage under `mib_admin_content_v1`. Public pages read saved content via `useContent` (seed fallback), never constants directly. Save guards: the detail hook never clobbers a dirty draft on upstream changes, the initial fetch never overwrites a page saved since mount, and Supabase mode drops stale localStorage entries for remote pages (see `ContentProvider.jsx`, `useContentDetail.js`).
- **Services catalog model:** the services page stores a canonical `catalog` tree (Categories → Sub-Categories → Items) built by `buildServicesCatalog` in `src/services/content.js`, which also regenerates the legacy keys (`serviceCollections`/`photoboothPackages`/`blissfulNestPackages`) from it. Admin Categories/Sub-Categories/Items screens write the tree; public Services/Homepage/Footer read the tree (never the legacy keys). Category titles are unique (enforced by `validateCategory` in `src/pages/admin/ServicesCMS/catalog.js`, case-insensitive). Homepage cards are the catalog categories — one card per category — with per-card CMS overrides merged by category id, then by title (`buildServiceCards`/`findServiceOverride` in `src/services/homepageCards.js`); an override that resolves to no category never renders. The admin Homepage Services section (`HomepageServicesPage`/`HomepageServiceDetail`) edits those same categories, so adding a service there creates a category and deleting a card deletes its category (with the move-children guard). Footer `Services` links follow the live catalog unless customized in Settings.
- **FAQ module is the first fully relational CMS-driven module:** `src/services/faqs.js` is the single source of truth for every FAQ-related piece of content on the public `/faqs` page. With Supabase configured, the admin module (`src/pages/admin/FAQsCMS/`) reads/writes `faq_categories`, `faqs` and the single-row `faq_page` (hero + CTA) through the main `supabase` client, and the public page reads published rows through the session-less `publicSupabase` client — no deploy needed, changes appear on next fetch. Without `.env` both sides fall back to a demo store in localStorage (`mib_admin_faqs_v1`) seeded from `src/constants/faqs.js`, so the full admin → public flow still works locally. FAQ archiving is a soft delete (`deleted_at` + `is_published = false`); deleting a category with FAQs requires reassigning them first (never orphaned). Public reads return only published, non-archived rows ordered by `display_order`; categories with zero published FAQs are omitted. The public page shows skeletons while loading and falls back to the seed copy for hero/CTA if the fetch fails (with a retry in the accordion area). `getActiveFaqs`/`getActiveCategories` in `src/constants/faqs.js` are now only used as seed source (and by `src/constants/services.js`) — the public page no longer imports them.
